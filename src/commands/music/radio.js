...
**34: ** [\`${Radiostations[34 - 1].split(" ")[0]}\`](${
    ...
    if (isNaN(args[0])) {
      return message.channel.send(
        new MessageEmbed()
          .setColor("#F0EAD6")
          .setTitle(
            `Not a valid radio station please use a Number between \`1\` and \`${Radiostations.length}\``
          )
      );
    }
    ...
    message.channel.send(
      `**<:sh_search:799392386564227103>  Searching \`${
        Radiostations[i - 1].split(" ")[0]
      }\`** <:sh_radio:854122485049982986>`
    );
    //mute yourself
